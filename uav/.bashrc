...
function run450(){
    ...
    tmux new-session -s cdrone\; set -g mouse on\; \
        ...
        send-keys "sleep 5; rosrun pd_control ROSI2.py"\;			
}

# Activate multiple terminals for mission 1
function m1run450(){
    echo "Starting Tmux session for EGH450 UAV control Mission 1"
    sleep 2
    tmux new-session -s cdrone\; set -g mouse on\; \
        send-keys "roscore" C-m\; split-window -h -p 85\; select-pane -t 0\; split-window -v\; \
        send-keys "htop" C-m\; select-pane -t 2\; split-window -h\; select-pane -t 2\;\
...
# Activate multiple terminals
function m2run450(){
    echo "Starting Tmux session for EGH450 UAV control Mission 2"
    sleep 2
    tmux new-session -s cdrone\; set -g mouse on\; \
        send-keys "roscore" C-m\; split-window -h -p 85\; select-pane -t 0\; split-window -v\; \
        send-keys "htop" C-m\; select-pane -t 2\; split-window -h\; select-pane -t 2\;\
